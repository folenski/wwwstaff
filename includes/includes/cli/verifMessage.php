<?php
/**
 * Permet de voir si il y a des messages pour avertir l'admin
 *
 * @author  Mario Ferraz
...
$flag=0;
$req=$TableMsg->select("*", compact(["flag"])) . " ORDER BY dateheure DESC";
echo " requete {$req} ";

$siteDB->openCur($req);

while ( $enr = $siteDB->fetchCur($req) ) {
    echo "{$enr["nom"]} | {$enr["mail"]} | {$enr["message"]} | \n";
}
exit;
